...
- `--chars02`: 두 번째 줄 문구 (기본값: `습니다`)
	- 인자 없이 실행하면 기본값 `고맙` / `습니다` 조합으로 2줄 이모티콘을 생성합니다.
	- `--chars01`만 지정하면 `--chars01` 문구만 사용해 단일 줄 이모티콘을 생성합니다.
	- `--chars01`과 `--chars02`를 함께 지정하면 2줄 이모티콘을 생성합니다.
...
- 단일 줄: `dist/문구1.png`
- 두 줄: `dist/문구1_문구2.png`
- 예: `--chars01 감사` -> `dist/감사.png`
